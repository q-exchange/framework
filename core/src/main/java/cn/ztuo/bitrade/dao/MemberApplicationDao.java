...
public interface MemberApplicationDao extends BaseDao<MemberApplication> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update member_application set audit_status=1,kyc_status=2,id_card=NULL where member_id=:memberId",nativeQuery =
            true)
    Integer updateMemberApplicationStatus(Long memberId);
}
