...
public interface MemberWalletDao extends BaseDao<MemberWallet> {
    ...
    @Query(value = "SELECT id, member_id, coin_id, frozen_balance, address, version, is_lock, release_balance, SUM(balance) AS balance FROM member_wallet WHERE member_id = :userId GROUP BY coin_id", nativeQuery = true)
    List<MemberWallet> getSumByCoin(@Param("userId") Long userId);

    @Query(value = "SELECT * FROM member_wallet WHERE member_id = ?1 AND coin_id = ?2", nativeQuery = true)
    MemberWallet getAssetsForCoin(Long memberId, String coinId);

    MemberWallet getByMemberIdAndCoin(Long memberId, Coin coin);
