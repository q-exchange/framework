...
//    OtcWallet findOtcWalletByMemberIdAndCoinId(Long memberId,String coinId);
    ...
    @Query(value = "SELECT id, frozen_balance, release_balance, is_lock, member_id, version, coin_id, SUM(balance) AS balance FROM otc_wallet WHERE member_id = :userId GROUP BY coin_id", nativeQuery = true)
    List<OtcWallet> getSumByCoin(@Param("userId") Long userId);
