...
public class ExchangeOrderController extends BaseAdminController {
    ...
    private Predicate getPredicate(ExchangeOrderScreen screen) {
        ...
        if(StringUtils.isNotEmpty(screen.getRobotOrder())){
            if("0".equalsIgnoreCase(screen.getRobotOrder())){
                booleanExpressions.add(qExchangeOrder.orderResource.eq(ExchangeOrderResource.ROBOT));
            }else {
                booleanExpressions.add(qExchangeOrder.orderResource.ne(ExchangeOrderResource.ROBOT));
            }
        }
