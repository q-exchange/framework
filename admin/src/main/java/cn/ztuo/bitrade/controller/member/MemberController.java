...
public class MemberController extends BaseAdminController {
   ...
   /* @RequiresPermissions("member:audit-business")
    ...
    public MessageResult outExcel(
        ...
        try {
            new FileUtil<>().exportExcel(request,response,list,"member");
        } catch (Exception e) {
            log.info("导出异常={}",e);
        }
     ...
     * 4.重置实名认证（未实名状态）
     */
    @RequiresPermissions("member:pwd-reset")
    @RequestMapping("reset/pwd")
    @AccessLog(module = AdminModule.SYSTEM, operation = "重置会员密码")
    public MessageResult resetPwd(@RequestParam("memberId") Long memberId,@RequestParam("type") String type) throws Exception{
        isTrue(!StringUtils.isEmpty(type), "类型为空");
        Member member = memberService.findOne(memberId);
        //重置登录密码（随机）
        String randomStr = GeneratorUtil.getNonceString(8);
        //重置交易密码（随机）
        String random = GeneratorUtil.getNonceString(6);

        if ("1".equals(type)){
            //重置登录密码（随机）
            String newPwd = Md5.md5Digest(randomStr + member.getSalt()).toLowerCase();
            member.setPassword(newPwd);
            memberService.save(member);
            return success("重置成功",randomStr);
        }else if ("2".equals(type)){
            //重置交易密码（随机）
            String newPwd = Md5.md5Digest(random + member.getSalt()).toLowerCase();
            member.setJyPassword(newPwd);
            memberService.save(member);
            return success("重置成功",random);
        }else if ("3".equals(type)){
            //重置Google认证（未开始状态）
            member.setGoogleState(0);
            member.setGoogleKey(null);
            ...
            //更新认证记录为未通过
            memberApplicationService.updateMemberApplicationStatus(memberId);
        }else {
            return error("参数异常");
        }
        memberService.save(member);
