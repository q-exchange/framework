...
public interface LeverWalletRepository extends JpaRepository<LeverWallet, String>,
    ...
    @Query(value = "SELECT id, member_id, member_name, mobile_phone, email, version, frozen_balance, is_lock, status, coin_id, lever_coin_id, SUM(balance) AS balance FROM lever_wallet WHERE member_id = :memberId GROUP BY coin_id ", nativeQuery = true)
    List<LeverWallet> getSumByCoin(@Param("memberId") Long memberId);
