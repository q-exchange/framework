...
public class MarketController extends BaseController{
    ...
    @RequestMapping(value = "all/volume",method = RequestMethod.GET)
    public MessageResult query24HVolume(){
        ...
        for(ExchangeCoin coin:coins){
            ...
            if(processor != null) {
                CoinThumb thumb = processor.getThumb();
                if(thumb!=null){
                    BigDecimal volume = thumb.getVolume();
                    BigDecimal usdRate = thumb.getUsdRate();
                    BigDecimal usdVolume = volume.multiply(usdRate);
                    totalVolume = totalVolume.add(usdVolume);
                }
            }
        }
        mr.setData(totalVolume.toString());
        return mr;
    }
}
