...
import cn.ztuo.bitrade.hbapi.request.CreateOrderRequest;
...
//    static final String API_HOST = "api.huobi.pro";
    ...
    public ApiClient(String accessKeyId, String accessKeySecret, String privateKey,boolean proxyFlag) {
        this.accessKeyId = accessKeyId;
        this.accessKeySecret = accessKeySecret;
        this.privateKey = privateKey;
        this.assetPassword = null;
    ...
    public MergedResponse merged(String symbol) {
        HashMap map = new HashMap();
        map.put("symbol", symbol);
        MergedResponse resp = get("/market/detail/merged", map, new TypeReference<MergedResponse<Ticks>>() {
        });
        return resp;
    }
...
//  }
    ...
    <T> T call(String method, String uri, Object object, Map<String, String> params,
        ...
        try {
            ...
            log.info("是否开启+{}===={}",this.proxyFlag,this.accessKeyId);
            if(this.proxyFlag){
                Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("127.0.0.1", 1087));
                log.info("代理开启={},",client);
                client =  client.newBuilder().connectTimeout(CONN_TIMEOUT, TimeUnit.SECONDS)
                        .readTimeout(READ_TIMEOUT, TimeUnit.SECONDS)
                        .writeTimeout(WRITE_TIMEOUT, TimeUnit.SECONDS)
                        .proxy(proxy).build();
                log.info("代理开启结束={},",client);
            }
